...
eset <- GSE14764_eset
class_labels1 <- phenoData(eset)@data$summarygrade

eset <- GSE17260_eset
class_labels2 <- phenoData(eset)@data$summarygrade

eset <- GSE9891_eset
class_labels3 <- phenoData(eset)@data$summarygrade
na_filter <- !is.na(class_labels3)
class_labels3 <- class_labels3[na_filter]

df3 <- df3[common_genes,na_filter]
...
aco_modules1 <- readRDS('aco_gamma_trial_14764_40.rds')
aco_modules2 <- readRDS('aco_gamma_trial_17260_40.rds')
aco_modules3 <- readRDS('aco_gamma_trial_9891_40.rds')

gs_modules1 <- readRDS('gs_results_14764.rds')
gs_modules2 <- readRDS('gs_results_17260.rds')
gs_modules3 <- readRDS('gs_results_9891.rds')
...
processAcoStats <- function(aco_modules, limma_adj_pvals, get_min_by, cutoff, adj_method){
...
min_stats_new1 <- processAcoStats(aco_modules1, limma_adj_pvals1, 'p_decays', 0.05, 'BH')
min_stats_new2 <- processAcoStats(aco_modules2, limma_adj_pvals2, 'p_decays', 0.05, 'BH')
min_stats_new3 <- processAcoStats(aco_modules3, limma_adj_pvals3, 'p_decays', 0.05, 'BH')
min_stats_new1 <- min_stats_new1[rownames(min_stats_new1) %in% rownames(min_stats_new2),]
min_stats_new2<- min_stats_new2[rownames(min_stats_new2) %in% rownames(min_stats_new1),]
min_stats_new3<- min_stats_new3[rownames(min_stats_new3) %in% rownames(min_stats_new1),]
